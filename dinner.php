	...
	<div class="pure-g">
        ...
        else
        {
           ...
           /* if (isset($userData) && isset($dateValue)) {
			...
			// ($weeks == 0 ? '<br /><br />' : )
			echo '<input type="button" class="alt_btn" value="Vorige week" onclick="window.location.href=\''.$settings['site_url'].'dinner.php?gid='.$group->id.'&p='.($weeks -1).'/\'"/> ';
			echo '<input type="button" class="alt_btn" value="Volgende week" onclick="window.location.href=\''.$settings['site_url'].'dinner.php?gid='.$group->id.'&p='.($weeks +1).'/\'"/><br /><br />';
            echo '<div class="double-scroll">';
